    ...
    for (j=0; j<= jello.resolution-1; j++)
      for (k=0; k<= jello.resolution-1; k++)
      {
        ...
        jello.forceField[i * jello.resolution * jello.resolution 
          + j * jello.resolution + k].x = double(rand() % 6 - 3);
        jello.forceField[i * jello.resolution * jello.resolution 
          + j * jello.resolution + k].y = double(rand() % 6 - 3);
        jello.forceField[i * jello.resolution * jello.resolution 
          + j * jello.resolution + k].z = double(rand() % 6 - 3);
      }
  ...
  for (i=0; i<=7; i++)
    for (j=0; j<=7; j++)
	    for (k=0; k<=7; k++)
      {
        ...
        if ((i==7) && (j==7) && (k==7))
        {
          jello.p[i][j][k].x=1.5 + 1.0 / 7;
	    	  jello.p[i][j][k].y=1.5 + 1.0 / 7;
		      jello.p[i][j][k].z=1.5 + 1.0 / 7;
        }
