...
void showBoundingBox()
{
		  ...
		  for (j = 0; j <= jello.resolution - 1; j++)
			  for (k = 0; k <= jello.resolution - 1; k++)
			  {
				  if (k % 10 == 0)
				  {
					  ...
					  glVertex3f(x + jello.forceField[i * jello.resolution * jello.resolution + j * jello.resolution + k].x * 1,
						  y + jello.forceField[i * jello.resolution * jello.resolution + j * jello.resolution + k].y * 1, z + jello.forceField[i * jello.resolution * jello.resolution + j * jello.resolution + k].z * 1);
				  }
  ...
  /* Show the moving direction of the cube */
  if (direction == 0)
  {
	  ...
	  for (i = 0; i <= 7; i++)
		  for (j = 0; j <= 7; j++)
			  for (k = 0; k <= 7; k++)
			  {
				  if (jello.v[i][j][k].z != 0 && k % 5 == 0)
				  {
					  glColor4f(0.0, 1.0, 0.0, 0);
					  double v_length = sqrt(jello.v[i][j][k].x * jello.v[i][j][k].x + jello.v[i][j][k].y * jello.v[i][j][k].y + jello.v[i][j][k].z * jello.v[i][j][k].z);
					  glVertex3f(jello.p[i][j][k].x, jello.p[i][j][k].y, jello.p[i][j][k].z);
					  glColor4f(0.0, 1.0, 0.0, 0);
					  glVertex3f(jello.p[i][j][k].x + (jello.v[i][j][k].x / v_length) * 1, jello.p[i][j][k].y + (jello.v[i][j][k].y / v_length) * 1, jello.p[i][j][k].z + (jello.v[i][j][k].z / v_length) * 1);
				  }
