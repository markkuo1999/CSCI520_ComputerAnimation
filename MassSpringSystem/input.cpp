...
void keyboardFunc (unsigned char key, int x, int y)
{
  switch (key)
  {
	...
	case 'm':
		...
		break;
	case '.':
		for (int i = 0; i <= jello.resolution - 1; i++)
			for (int j = 0; j <= jello.resolution - 1; j++)
				for (int k = 0; k <= jello.resolution - 1; k++)
				{
					jello.forceField[i * jello.resolution * jello.resolution
						+ j * jello.resolution + k].x*=1.1;
					jello.forceField[i * jello.resolution * jello.resolution
						+ j * jello.resolution + k].y*=1.1;
					jello.forceField[i * jello.resolution * jello.resolution
						+ j * jello.resolution + k].z*=1.1;
				}
		break;
